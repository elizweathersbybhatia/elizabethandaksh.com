...
## DONE (session 14 — 2026-06-15)
56. **Baraat & Reception color palette updated** — replaced the original white/beige/maroon swatches with more vibrant Indian wedding colors. Kept: black (`#171717`), navy (`#153B66`), gold (`#B8943E`). Replaced: white → crimson (`#C8102E`), beige → rani pink (`#D4327A`), maroon → coral/organza (`#E87A54`). Change is in the `.attire-card-palette` block on the Baraat & Reception `<article>` (line ~621).
57. **Photo marquee seamless loop fixed** — the JS-based scroll loop had a frame-timing gap where the background briefly showed as it reset position. Replaced entirely with a CSS `@keyframes marquee-scroll` animation using `translateX(-50%)` on the `.photo-marquee-track`. The browser handles the loop at the compositor level, zero visible gap. Desktop: 90s duration, mobile: 60s. JS tick/drag/pause code removed. `overflow:hidden` on `.photo-marquee` with explicit `height:340px` (desktop) / `240px` (mobile).
58. **FAQ icons replaced with custom PNGs** — all 9 FAQ accordion items now have real icon images instead of Unicode symbol characters. Icon pack saved to `assets/icons/faq/` (committed). Mapping: `dress_hanger.png` → attire, `plane.png` → travel, `bed.png` → accommodation, `car.png` → parking, `holding_hands.png` → plus one, `sun.png` → weather, `serving_platter.png` → food, `martini_glass.png` → open bar, `food_allergy.png` → dietary. Car icon was user-supplied (AI-generated sketch of a wedding car with cans). All source PNGs were tightly re-cropped (alpha > 30 threshold) to remove excess transparent padding so the drawings fill the icon circles. CSS: `.faq-icon` has `overflow:hidden`, `.faq-icon img` is `38×38px` with `object-fit:contain` (desktop); `30×30px` (mobile).
...
- 2026-06-15 (session 14): Items 56–58. Baraat & Reception palette updated to vibrant Indian colors. Photo marquee loop fixed with CSS animation (no more background flash). All 9 FAQ icons replaced with PNG images; car icon added for parking; icons re-cropped to remove transparent padding so they fill circles.
